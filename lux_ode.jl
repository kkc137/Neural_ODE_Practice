using DiffEqFlux, Zygote, MLDataUtils, NNlib, OrdinaryDiffEq, Test, Lux, Statistics,
    ComponentArrays, Random, Optimization, OptimizationOptimisers
using MLDatasets: MNIST
using MLDataUtils: LabelEnc, convertlabel, stratifiedobs

#here because compatibility of metal, all data are cpu
#because raw data have problem of gdev, can not fix
...
#     if (iter % 10 == 0)
#         @info "[MNIST CPU] Accuracy: $(accuracy(m, zip(x_train, y_train), ps, st))"
#     end
#     return false
# end

# Train the NN-ODE and monitor the loss and weights.
res = Optimization.solve(opt_prob, opt, zip(x_train, y_train))
@test accuracy(m, zip(x_train, y_train), res.u, st) > 0.8
@info "Accuracy: $(accuracy(m, zip(x_train, y_train), res.u, st))"
# @info "res = $res"